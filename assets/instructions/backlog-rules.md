...
⚠️ **ID は、番号系統を共有する全ての backlog の最大値 + 1。起票前に全てを grep する。**
共有先は `backlog.md` の冒頭に列挙してある（書かれていなければ `backlog.md` 1 つだけ）。

```bash
grep -ohE "BL-[0-9]+" <番号系統を共有する全ての backlog> | sort -t- -k2 -n | tail -1
```

- **片方だけを見て採番しない。** 衝突した番号は、後から参照した人が別の項目を引く
  （コメントや文書の「BL-xxx」がどちらを指すか分からなくなる）
- 最大値は grep で取る。見出しの並びは ID 順ではないので、末尾の見出しを最大とみなさない
- この規則は宣言される前、書き手の注意だけで守られていて、**実際に破られた**。注意ではなく手順として毎回やる
