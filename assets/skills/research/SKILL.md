...
`## Modify` だけは **1 行に 1 パス**とし、長いパスも折り返さない。パーサは箇条書き
1 行から最初のインラインコードしか採らないため、2 個目以降は黙って捨てられ、行頭が
`- ` でない継続行も丸ごと捨てられる。パスは checkRepoRoot からのフルパスを使い、
`KY/` や `CA/` のような接頭辞の略記を使わない（略記や glob を展開する機構はない）。
行番号や理由は 2 個目以降のインラインコードか地の文に置いてよい
（例: `- \`src/a.ts\`（\`:49-50\` 付近）`）。この規則は validator が読まない
`## Read` / `## Reference` / `## Ignore` には適用しない。
