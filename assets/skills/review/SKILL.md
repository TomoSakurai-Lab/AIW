...
### `### Files To Modify` の書き方

`### Files To Modify` は **1 行に 1 パス**とし、長いパスも折り返さない。各パスは
checkRepoRoot からのフルパスをインラインコードで書き、接頭辞の略記を使わない。
パーサは各箇条書きの最初のインラインコードしか採らず、継続行や略記を展開しない。
行番号や理由は 2 個目以降のインラインコードか地の文に置いてよい。fix の `diff-scope` は
`onViolation: halt` なので、この書式を外すと report では済まず、その場で停止する。
