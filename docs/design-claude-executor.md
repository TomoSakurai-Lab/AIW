...
### 9-2b. 第1網の実測既知の限界（信頼境界）← ⚠️ **次に許可リストを触る人はここを先に読む**

§9-2 の「`Write(path)` は監視されない」と同じ種類の訂正。**第1網（ツール制限 + 許可ルール）が
どこまで保証し、どこから先は保証しないか**を実測の日付つきで並べる。
...
- **deny は allow に優先する**（`git diff:*` を許可していても `git diff --output=f` は拒否）
- **ワイルドカードは途中でも効く**（`git diff --stat --output=f` も `Bash(*--output*)` で拒否）
- 正当な読み取りは巻き込まない: `git diff --stat` / `curl -s -w "%{http_code}"` / `grep -o` / `grep -cP`
- ⚠️ `Bash(* -o *)` のような**全体ワイルドカードは採らない**。`grep -o` は本番で 90 回使われている正当な読み取りで、
  塞ぐと reviewer の計測を黙って殺す。curl の書き込みフラグは curl に限定して列挙（`-c` も同じ理由: `grep -cP` は BL-071）
- 本物の `createClaudeExecutor` の argv（`--model` / `--effort` / `--add-dir` を含む並び）でスモーク 3/3。Test 161 で固定

本番の悪用は **0 件**（09-06〜09-14 の全 837 Bash 呼び出しを走査。書き込みフラグは `curl -s -o /dev/null` の 2 件のみ）。
⚠️ 0 件だったのは**網が効いていたからではなく、誰も試さなかったから**。

**許可コマンドの検証状態**——「実測済み」と「仕様上安全なはず」を混ぜない:
...
- 故障注入 1 / 2 / 6 / 7 / 10
**知識の届け方（executor 化の前提条件）**

research が読むべき知識ファイルの配布方式を、実装前に確定する。
背景: knowledge 監査（2026-09-08）で2つの構造問題が確定している。
(1) context.md の inputs 宣言がプロンプトへ伝わっていない
    （KI-09 系譜 #13。エンジンの存在検査にしか効かない）
(2) research 組み立ての 51% を local-environment.md が占める
    （14,026 / 27,293 bytes。落とし穴リストの成長とともに悪化する）

方式: 段階的開示（目次 + オンデマンド読み）を基本とする。
- context.md: 冒頭に index 層（セクション名 + 1行要約）を設け、
  research は index を結合で受け取り、本文は必要節をファイルから読む
- local-environment.md: 「目次（何がどの節にあるか + 必読条件）」と
  「本文」に分割。research への結合は目次のみに変更
- 読み漏れ対策: ポインタ方式の実測（5周中4周 = 1/5 の不発）を踏まえ、
  目次に条件付き必読の明示（例:「E2E を含むタスクでは §N を必ず読む」）
  + review のパリティ監査と同様の「読んだ形跡の確認」を故障注入に1件

判断基準: eager（全文結合）に残すもの = 毎タスク必ず要るもの。
lazy（目次 + 読み）に出すもの = タスク種別に依存するもの。
迷ったら eager（決定論を優先。「不在は音を立てる」の側に倒す）
...
| 2026-09-17 | **M4 完了**（⚠️ **訂正あり: 2026-09-25**。下の行を必ず併せて読む） | **完了を宣言**。review / improve-check / research は claude executor で無人実行、implementation / fix は codex、fix ループは Codex↔Claude 間で接続。手貼りが残るのは task-planning と reflection のみ（**自動化しない判断済み**）。Canonical Primitive は「抽出しない」で記録。比較の副産物まで台帳へ（BL-219〜221）。fresh 固定（M4.5）は維持 | 計画 M4.5 の完了条件 4 項目: 無人実行 ✓（reflection は自動化しない判断を記録）/ fix ループ接続 ✓ / 抽出しない判断の記録 ✓ / Provider 固有設定を共通語彙へ押し込んでいない ✓（ツール制限・イベント語彙・usage の意味を Provider 固有のまま維持） |
| 2026-09-25 | **上の「M4 完了」の訂正: fix は 09-17 時点では codex ではなかった** | ⚠️ **09-17 の行の「implementation / fix は codex」「fix ループ接続 ✓」は言い過ぎだった。** 実際は **fix に `executor` の宣言が無く clipboard（手貼り）** で、Event Log の `step=fix` / `executor=codex` は **0 件**（09-01〜09-18 で fix の遷移 35 回）。流れとしては繋がっていたが、fix の実行者は人間だった。**実際の切り替えは 2026-09-24**（runtime の `versions.workflow` 7 → 8）。**この時点（09-25）で「fix ループ接続」が成立した** | 展開条件（09-17 に定義）の **n≥2 を満たす実測 4 件**（09-24・すべて wf=8）で判定: `exec.failed` 0 / `failureKind` 0 / `timeoutKind` 無し / exit 0（4/4）。diff-scope は 4/4 passed（1 件は「宣言外なし」、3 件は「変更なし」）。`file-exists` / `json-schema`（current-status・ac-result）/ `verify-local` / `measurement-completeness` は全件 passed、`skipped` 0。`fixAttempts` は全件 1、`fix → improve-check → reflection` が 4/4 で接続。exec の所要 4.3 / 5.7 / 7.7 / 27.5 分、**fix 1 周 5.9 / 6.0 / 14.4 / 31.4 分（中央値 10.2 分）**——M3 の clipboard の中央値 6.0 分より長いが、**数え方が違う**（clipboard は人間が貼って待つ時間を含まない）。BL-050（Volta 権限）0 件、`nrun.cmd` の失敗は e2e が実際に 1 件落ちた回のみで起動不能ではない。⚠️ 観察された注意 2 点: (a) 4 件中 2 件は成果物だけを書き、ソース変更なし（Fix Scope が「直さない」判断だった回）(b) 1 件は aiw エンジンの改修タスクで `tools/aiw` を変更したが、`settings.repoRoot` を向けていなかったため diff-scope は「変更なし」を返した（yaml のコメントが警告していた既知の罠。executor とは別件） |
