...
> 分類表の**過去行**（🗑 削除済みの理由欄など）に残る `.ai-workflow2` は現 `.ai-workflow` を指す。
> 当時の判断の記録なので書き換えていない。
...
| 指示の要約 | 分類 | 移行先 | 状態 |
| --- | --- | --- | --- |
| `.ai-workflow/` 配下が対象ディレクトリ | 恒久規則 | Instructions | ✅ |
...
- 対象ディレクトリの注記は 5 ファイルにあった → `coding-rules.md` の 1 箇所
  （2026-09-02 追記: その 1 箇所も**さらに 1 行へ縮約**した。「旧 `.ai-workflow/` と混同するな」の
  半分はレガシー撤去で根拠を失ったため削除。下記「免責ブロックの削除」を参照）
...
## 免責ブロックの削除（2026-09-02）

`task-planning.md` / `review-audit.md` の冒頭にあった次の免責ブロックを削除した。

> **対象ディレクトリ**: ワークフローのファイルはすべて `.ai-workflow2/` 配下（末尾 `2`）。
> （…）同名ファイルを持つ旧 `.ai-workflow/`（`2` なし）は別物で、参照・変更しない。

`coding-rules.md` の同趣旨の節（旧ディレクトリへ書き込まない・原則読まない）も削除し、
**「成果物・状態ファイルはすべて `.ai-workflow/` 配下にある」の 1 行**へ縮約した。

**由来**: v0.2 レガシー `.ai-workflow/` を撤去したので、
「2つあるから混同するな」という指示は**指す先が消えた**。
残すと KI-09 の系譜そのもの（宣言はあるが効かない／読んだ人が実在すると信じる）になる。
...
| 追加先 | 内容 |
| --- | --- |
| research Skill | 「兄弟機能の棚卸し（パリティ表）」節。同一データへの既存類似操作を識別子 grep で列挙 → 差集合を `research-findings.md` の `# Delta` 配下 `## Sibling Parity` へ → 「適用」は AC Matrix に接続 / 「対象外」は `# Open Decisions` に固定 / 見つからなければ「兄弟なし（grep: 識別子 / 0件）」を1行 |
| review Skill | Specification Coverage Audit に「パリティ監査」1項目。表も「兄弟なし」も無いのに兄弟が明らかに存在する場合は `research起因` の Major |
| local-environment（runtime のみ） | この環境の識別子ペア（`getSelectedRows()` の呼び出し元 = 行操作 / `cellRangeRef`・`extraCellsRef` の消費箇所 = セル操作）と、同構造が 2 箇所に複製されている記録 |

### 由来

行選択に後からセル選択を足したグリッドで、行選択が持つ操作 ≈15 のうちセル選択にも効くのは
copy / cut / paste の 3 つだけだった（2026-09-04 実測、`JikkouYosanSakuseiGrid.tsx`）。
仕様書はどちらにも触れておらず、実装は書かれたものを正しく作っていた。
「兄弟と同じに動く」という暗黙の期待は Source Requirements に載らないので、
research が棚卸しして AC に落とさない限り実装されない。

### 設計の分離

- 汎用の手順は Skill、環境固有の識別子は local-environment（M2 の汎用 / 環境固有の分離）
- 発動条件は「類似物を足すとき」という**判断**ではなく「識別子を grep する」という**手順**
  （KI-09「宣言はあるが効いていない」のプロンプト版を避ける）
- 「兄弟なし」の記録まで要求するのは、review が「棚卸しの結果なし」と「棚卸し忘れ」を
  区別するため（不在は記録で示す）
- 構造対処（選択の正規化層）は BL-136。プロンプト層は当面の運用対処

## Fix 原因分類の3値化とパリティ指摘の Backlog 既定（2026-09-08）

### 変更した指示（**既存の語彙の差し替え**。新規の手順追加ではない）

| 変更先 | 内容 |
| --- | --- |
| review Skill（Specification Coverage Audit） | Fix 原因分類を 2値 → **3値**。`実装起因` / `research起因（指示が誤り）` / `research起因（指示が不足）`。「仕様が未定義だった」を独立カテゴリにしない注記付き |
| review Skill（パリティ監査） | パリティ由来の兄弟未対応は **Scope 内なら Fix Scope / Scope 外なら Backlog**（Trigger 必須・既定は「次に当該機能を触るタスクの前」）。判定基準は `context-package.md` の Scope 宣言、迷えば Backlog 側 |

版: `skills.review` 4→5（assets / runtime 両方）。prompts / templates / schemas / workflow は変更なし。

### 由来

**分類が弁別力を失った。** review を claude executor 化して以降、Fix が出たタスクの
`research起因` が 5/5 = 100% になった。原因は品質低下ではなく分類の構造にある:

1. 自動レビュアーは 2択の分類を**全件へ律儀に適用する**。人間なら「どちらとも言い難い」を
   書かずに済ませるところを、必ずどちらかへ落とす。**指示が無かったもの**は
   「実装は指示どおり」なので消去法で `research起因` に落ちる
2. パリティ監査は**構造上 research を名指しする**（棚卸しは research の仕事なので、
   漏れは必ず research 起因になる）

**100% は情報量ゼロ。** どこを直せば減るのかが読めないので、`research起因` を
**誤り**（指示はあったが内容が違う）と **不足**（指示自体が無かった）へ割る。
この内訳が M4 段階3（research の自動化）の判断材料になる——
「不足」が多いなら棚卸しの網を増やす話、「誤り」が多いなら research の検算の話で、
打ち手が別だから。

パリティ指摘の Backlog 既定は別の問題への対処。パリティ監査は**兄弟機能の未対応**を
拾うので、放っておくと今回のタスクと無関係な範囲まで Fix Scope に入る。
**scope の膨張は `fixAttempts` の有界（初回 + maxRetries=2）と衝突する**——
入れた分だけ fix ループが伸び、上限に当たれば escalate で人間へ戻る。

### 設計の分離

- **分類の記載位置は変えない。** `## Specification Coverage Audit` の中のまま、語彙だけ差し替える
  （current-review.md の契約見出しは不変なので `artifact-contract` に影響しない）
- **「未定義」を独立カテゴリにしない。** 未定義を `# Open Decisions` へ落とすのは research の
  仕事なので、落ちていなければ「不足」。落ちていて人間が判断済みなら、その判断に従った
  実装の問題は「実装起因」。カテゴリを増やすほど消去法の逃げ場が増えるため、3値で止める
- **Scope 内/外の判定は既存の宣言（`context-package.md` の Scope）に載せる。**
  review に新しい判断基準を持たせない（BL-115 と同じで、review は scope を広げない）
- エンジン・validator・許可値・遷移はすべて不変。**プロンプト層のみ**

## 知識の届け方: 目次 + 条件付き必読（2026-09-11）

### 変更した分解（**結合するもの / 読みに行くもの**の線を引き直した）

| 情報 | 分類 | 移行先 | 状態 |
| --- | --- | --- | --- |
| プロジェクト知識（`context.md` 264 KB） | 恒久規則 | **読みに行く**（冒頭 `## 索引` → 条件に当たる節だけ Read） | ✅ |
| 環境の常時必要な手順（6 節 5,301 B） | 環境固有 | local-environment（**結合**。従来どおり） | ✅ |
| 環境のタスク種別依存の手順（3 節 9,651 B） | 環境固有 | **`instructions/local-environment-detail.md`（新設・結合しない）** | ✅ |
| 「どの条件でどれを読むか」 | 手順 | local-environment の `## 目次` / `context.md` の `## 索引` | ✅ |
| 「まず索引を読む」手順 | 手順 | **Skill**（research v5） | ✅ |
| 索引を本文と一緒に更新する規則 | 手順 | **Skill**（reflection v5） | ✅ |

### `local-environment-detail.md` の位置づけ（new-artifact-checklist の通し）

**成果物ファイルではなく恒久の知識ファイル**（`instructions/*.md` と同類）。
チェックリストの項目は**消さずに**、当てはまらない理由を残す:

| 項目 | 判断 |
| --- | --- |
| 生成 | **該当なし**——ステップが書くファイルではない（人間が書く恒久ファイル） |
| contract | **該当なし**。⚠️ **むしろ `workflow.yaml` に宣言してはいけない**——宣言すると結合され、分割した意味が消える |
| archive | **該当なし**——タスク固有の記録ではない。archive すると毎タスクのコピーが増える |
| 掃除 | **該当なし**——「作業中の器」でも「そのタスクの記録」でもない。タスク終了時に何もしない（`instructions/*.md` と同じ） |
| 分類表 | ✅ この節 |
| 許可リスト | **該当なし**——codex は読むだけで書かない |
| パス規約 | ✅ 目次からの参照は **runtimeRoot 相対**（`instructions/local-environment-detail.md`）。本文中のパスは移設前の表記のまま（checkRepoRoot 相対 / 絶対）で、意味は変えていない |
| 2周目のテスト | **形を変えて該当**——「2件目が1件目を上書きする」型ではなく、**目次と本文のドリフト**（節を足して目次に書き忘れる）が 2 周目に効く欠陥。機械検査は BL-118 として起票 |
| 実配置の模倣 | ✅ テストの土俵ではなく**本番の runtime で組み立てを実測**（28,233 → 21,832 B。⚠️ 当初 20,333 B と書いたのは research Skill の「0. 知識の取り込み」節を足す前の値で、2026-09-14 に訂正）。⚠️ ただし **「本番で読まれた実績」は 0 件**——`meta.filesRead` に `local-environment-detail.md` が出るまで「効いている」とは言わない |

### 由来

research の組み立て 28,233 B のうち **14,981 B（53.1%）**が local-environment で、
その大半は「`.cs` を触る」「E2E を含む」タスクでしか要らない手順だった（2026-09-11 実測）。
落とし穴リストは**増える一方**なので、放置すると比率は悪化し続ける。

より重いのは `context.md` で、**264 KB（≈66K tokens）あるのにプロンプトへは一切届いていない**。
`inputs` 宣言はエンジンの存在検査にしか効かず、`prompts/research.md` は Output 宣言だけ、
research Skill は `current-task.md` しか名指ししていなかった（KI-09 系譜 #13）。
clipboard 運用では人間が補っていたが、**executor 化すると知識ゼロで走る**。

### 設計の分離

- **eager / lazy の基準は「毎タスク必ず要るか」**。迷ったら eager（決定論を優先）
- 索引・目次は**読み手の側に置かない**（Skill に見出し一覧を再記述しない）。
  正本はファイル自身の冒頭で、Skill は「索引を読め」としか言わない
- ⚠️ **分割は読み漏れと引き換え**。実測でポインタ方式は **5 周中 4 周（= 1/5 の不発）**だったので、
  目次に**条件付き必読**を明示し、読んだ形跡を `meta.filesRead` に残す。
  記録は**観測であって判定ではない**（自動で咎めると、読まずに正解できた回まで止まる）
