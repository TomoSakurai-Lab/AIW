...
| **drive → auto の合流**（2026-09-25 追加） | drive の executor の質問に `a=ここから auto（無人区間の終わりまで）` を足す。`a` でその場で auto のループへ合流し、auto の停止条件・終了コード・ロック・再試行をそのまま使って終わる（drive 用には何も複製しない）。**一方向だけ**（auto から drive へ戻る経路は作らない。止まった後の次の手は停止理由の1行が教える）。**`a` は `auto: true` のステップに限る**（区間の規則は `autoIneligibility` の1箇所で、auto のループの A2 / A3 と共有。drive から入れば区間外も無人で回る、という抜け道を作らない）。ロックは `a` の瞬間に取る（drive はそれまでロックを見ない）。Test 230・231 |
