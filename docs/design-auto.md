...
4. 終端（complete）                       → A4
5. 未定義のステップ                       → A25
6. executor === "clipboard"               → A2
7. auto の区間外（step.auto !== true）    → A3
8. 予算                                   → A11
9. stale status（run 前検査）             → A17（exec の後、run の前に判定）
10. exec → run
```
...
| A13 | タイムアウト | `meta.timeoutKind` が idle / total | **1回だけ即再試行** | `⛔ executor 失敗(timeout ×2) implementation` | 同上 |
...
| タイムアウト（idle / total） | **1回** | 即時 | 1回で 5/6 が回復（§5）。1回あたり最大で総上限（30〜40分）+ kill の遅れがかかるので2回目は人に返す |
...
- 設定: `settings.autoRetry: { timeout: 1, transient: 3, waitsMs: [300000, 900000, 1800000], maxPerRun: 6 }`。
  値は `auto.started` イベントに記録する

**記録**: `auto.retry` イベント `{ runId, step, attempt, cause: "timeout-idle" | "timeout-total" | "transient", waitMs, message }`。
...
**推奨: D2 は M5 本体から切り離し、(a) の小枠の後に入れる**（未解決の論点 #2）。
...
| 範囲 | **起動1回の中だけ**（sticky にしない）。次のステップは pin したモデルから始める（未解決の論点 #4） |
| 使い切り | **auto の停止**（終了コード 4・state は変わらない） |
| claude のステップ上書き | `step.model` があればそれを起点に、`claudeModelFallbacks` を後ろに試す |

⚠️ **指示文の「全段使い切りで halt」は、auto の停止（終了コード 4）と読み替える**（未解決の論点 #3）。
...
engine: classifySituation(root, config) → Situation      ← nextSuggestion / drive / auto が共有
          | halted | awaiting-approval | checkpoint | terminal | unknown
          | clipboard-step | out-of-zone | stale-status | runnable

cli:    stepLoop(policy)
          drive policy: ゲートで y/n を聞く / halt で resume を聞く / 実行前に y/n を聞く
          auto  policy: ゲート・halt・clipboard・区間外で止まる / 実行は確認なし
```
...
| 判定の順序 | `nextSuggestion` の順（halted → 承認 → チェックポイント → 終端 → 不明 → …）に揃える。drive は今「ステップ未定義」を先に見るが、到達しうる状態では結果が同じなので、**載せ替えで挙動は変わらない**。これをテストで固定してから載せ替える |
...
2. **明示の完了申告を外すと、BL-116 の穴が静かな素通りになる**。research-findings.md のテンプレートは
...
**推奨: M5 には入れない。後続枠にする**（未解決の論点 #5）。

- 効果は auto の区間で観測上ゼロ、リスクは実在する（鮮度の証明の喪失・BL-116 の素通り化）
- 契約と validator の変更を含み、M5 のやらないことに当たる
- 着手条件: **BL-116 が解消済み**、かつ M5 の運用で区間内に invalid-status が再び出る
...
| 7 | タイムアウト → 再試行もタイムアウト | 1回だけ再試行して 4 |
...
| 22 | drive の載せ替え前後で、到達しうる全状態に対する判定が同じ | 判定器の抽出が挙動を変えないこと（課題E） |
...
- 再試行は **timeout 1回即時 / その他の transient 3回（5・15・30分）**。判断に失敗の結果ではなく auto 自身の中断フラグを使う（課題D）
...
- **D2**: executor に `transientCause` を足す小枠 → フォールバック本体
- **I**: BL-116 の解消 → status の機械導出
...
| # | 論点 | 選択肢 | 私の傾き |
| --- | --- | --- | --- |
| 1 | **区間の境界の宣言**（前提3 だけでは research を止められない・§3） | (a) ステップごとに `auto: true`（既定は false）/ (b) `settings.autoSteps: [...]` / (c) 導出規則で決める / (d) research を auto の対象から外すことをコードに書く | **(a)**。宣言がステップの横にあり、既定 false なので**新しい executor ステップが黙って無人区間に入らない**。clipboard のステップに書かれたら BL-219 の仕組みでロード時に知らせる。(c) は「review は入るが research は入らない」を構造から区別する規則が作れなかった（どちらも claude・ゲートあり） |
| 2 | **D2 を M5 本体に入れるか** | M5 に入れる（executor の変更を例外として認める）/ **後続枠**（`transientCause` の小枠の後） | **後続枠**。判定の材料が executor の返り値に無い。実測の発生は 2 件で、D の待機つき再試行で回復する時間尺度だった |
| 3 | D2 の「全段使い切りで halt」 | エンジンの halt（state を書く）/ **auto の停止（終了コード 4・state は変わらない）** | **auto の停止**。exec の失敗は state を変えない（不変条件1） |
| 4 | D2 のフォールバックの範囲 | **起動1回の中だけ**（次のステップは pin から）/ 起動の残りすべて（sticky） | **起動1回の中だけ**。モデルが変わるステップを最小にする。容量不足が続いていれば各ステップで同じ手順を踏むので遅くなるが、記録は明快 |
| 5 | **I を M5 に入れるか** | M5 と同時 / **後続枠**（BL-116 の解消が前提） | **後続枠**。auto の区間での効果は観測上ゼロ、リスク（鮮度の証明の喪失・BL-116 の素通り化）は実在。契約と validator の変更も含む |
| 6 | 再試行の回数と間隔 | timeout 1回即時 / transient 3回（5・15・30分）/ 起動あたり上限 6 | この値で。容量不足の実測（数十分で回復）と、タイムアウト1回の重さ（最大 30〜40 分）から |
| 7 | 同時実行のロック | 入れる / 入れない | **入れる**。codex が同じ成果物を奪い合う事故は、起きてからでは切り分けにくい |
| 8 | 終了コードの割り当て | 0 / 1 / 2 / 3 / 4 / 5 / 130（課題G） | この割り当てで。既存の 0 / 1 / 2 の意味を変えない |
