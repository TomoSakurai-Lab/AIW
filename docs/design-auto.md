...
**実装で確定（2026-09-25・段階1）: 変わったのは表の1と2だけ。見落としの追加は無かった。**

| 確かめ方 | 結果 |
| --- | --- |
| 統一前の挙動を固定したテスト（コミット `a7e668f`・src は変更なし）を、書き換えずに新しいコードで実行 | **落ちたのは Test 190（表の1）と Test 191（表の2）の2本だけ**。drive の単一条件（188）・定義済みステップでの重なり（189）・next（192・193）は無傷 |
| drive / next に関わる既存テスト（21・22・23・34・106） | すべて無傷 |
| 変異テスト: 判定器の halted と承認待ちの順序をわざと入れ替える | **194（組み合わせ）・192（next）・189（実物の drive）の3本が同時に落ちた**。3経路とも順序を固定できていることの確認（緑のテストが検出力の証明にならない、系譜 #11 の教訓） |

実装で確かめた、表に無い細部（挙動は変わらない）:

- 表示の文言（halt の理由・ゲートの名前・終端の文言）は、`state` から直接読んでいた値を
  `Situation` が運ぶ同じ値に置き換えただけ
- 表の1の「承認待ち + 未定義ステップ」は、統一後は判断材料と承認ゲートを出し、却下するとループへ戻って
  そこで初めて「不明」と言って終わる。クリップボードにも executor にも進まない（Test 190 で確認）
...
| **drive の auto モード**（2026-09-25 追加・同日改訂） | drive の executor の質問に `a=ここから auto（人の番で drive に戻る）` を足す。`a` で auto モードに入り、無人区間のステップは確認なしで auto のループ（停止条件・終了コード・ロック・再試行をそのまま使う。drive 用には何も複製しない）に任せる。**auto が人の番（終了コード 0: 承認待ち・clipboard・区間外・完了）で止まったら drive の対話へ戻り**、ゲートでは drive が判断材料を出して承認を聞く（承認するのは人。auto は承認を呼ばない）。**異常停止（終了コード 0 以外）なら drive も終わる**。戻る判断は `returnsToDrive` の1箇所。戻る先は drive に既にある分岐だけで状態は増えない。単体の `aiw auto` は無対話のまま。端末では auto の実行中に打たれた入力を捨てる（ゲートを事前入力で通さない）。**`a` は `auto: true` のステップに限る**（区間の規則は `autoIneligibility` の1箇所で、auto のループの A2 / A3 と共有。drive から入れば区間外も無人で回る、という抜け道を作らない）。ロックは `a` の瞬間に取る（drive はそれまでロックを見ない）。Test 230・231・232 |
...
| 2026-09-25 | **段階2 の報告の承認**（人間） | 4分割への変更・コミット番号の不揃いを直さないこと・未記述の判断12点・BL-242 のスコープ外送り（Test 208 の補償で実害小）を承認 | auto キーを executor 別の表と分けたのは、KI-09 系譜 15例目（codex ステップの model が黙って無視される）の教訓の適用 |
| 2026-09-25 | **drive → auto の合流**（人間の承認・M5 の「drive は変更しない」を変更） | drive の executor の質問に `a=ここから auto` を足す。**制約 2 つ**: (1) 方向は drive → auto の一方向だけ。合流したら auto の停止条件で止まって終わる（drive へ戻らない）(2) `a` の対象は `auto: true` のステップに限る（区間外なら「無人対象外です。y/n で進めてください」と断る）。ロックは `a` の瞬間に取る | drive で見ながら進めていて「ここから先は無人でいい」と思ったとき、一度抜けて `aiw auto` を打ち直すのは、判定器を共有した意味に反する手間（課題E「同じ判定器の上の2モード」の配当）。双方向にすると状態遷移が倍になる。区間の規則を drive 側で緩めると、論点 #1 の「既定 false・黙って無人区間に入らない」が骨抜きになる |
| 2026-09-25 | **drive の auto モード: 人の番で drive へ戻る**（人間の承認・上の行の制約 (1)「一方向だけ」を改訂） | auto が**終了コード 0（人の番）で止まったら drive の対話へ戻る**。承認ゲート（research ② / review ③）では drive が判断材料を出して y/n を聞き、承認後に次が無人区間なら確認なしで auto を再開する。**終了コード 0 以外なら drive も終わる**（今までどおり）。制約 (2)「`a` は `auto: true` のステップに限る」は維持。単体の `aiw auto` は無対話のまま | 承認が要るゲートでは、auto で回していても人に入力を求めてほしい（人間の要望）。auto 自身に聞かせると「承認を呼ばない」（前提5・Test 202）と「スクリプトから呼んでも stdin で止まらない」（課題E）が崩れるので、聞くのは対話ループを持つ drive にした。「状態遷移が倍になる」懸念には、戻る先を drive に既にある分岐（ゲート・clipboard・区間外・完了）だけにし、戻るかどうかを終了コードの1点で決めることで応えた。人の番の後に drive が聞かずに auto を再び呼ぶ経路は無い（auto が 0 で止まるのは区間外か人の操作が要る状況だけ）ので、無限に回らない |
| 2026-09-25 | **BL-241 の優先度引き上げ**（人間） | deferred → **次の枠で対処**。BL-243 を統合し「**テスト実行の信頼性**」枠として1本にする。スコープ: (1) 方向を選ぶ（ファイル単位の分割実行を Skill の手順に / 上限の引き上げ / テストの高速化）(2) BL-243 の根治 (3) **`# fail 0` で判定する暫定運用を Skill に明記** | 放置すると aiw 側の全タスクで、テストを流すたびに上限に当たり AC の証拠がゼロになる。暫定運用が runtime の context.md にしか無いと、次の implementation が exit 1 を誤判定する |
