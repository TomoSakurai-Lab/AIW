...
⚠️ **#11 で型が1つ増えた。** #1〜#10 はいずれも「宣言はあるが**実行されない**」だったが、
#11 は **実行され、テストも通り、それでも一度も正しく機能していなかった**。
原因はテスト側で、**テストの前提が実装のバグと一致していた**（consumer root を
runtimeRoot 配下に作っていたので、runtimeRoot 起点で解決するバグと噛み合って通っていた）。

これは M3 で「テスト環境がリポジトリですらなかった」を直したのと同根で、
**テストが実配置を模倣していない**という一つの欠陥の別の現れ方である。

見分ける手立ては1つしかない。**緑のテストではなく、本番の実績を数えること**——
`passed` が 0 件なら、その検査は一度も機能していない。
`docs/new-artifact-checklist.md` の 9 点目がこの観点。
...
再発防止のチェックリストは **`docs/new-artifact-checklist.md`**（BL-102・2026-09-01 新設）。
**生成 / contract / archive / 掃除 / 分類表 / 許可リスト / パス規約 / 2周目のテスト /
テストは実配置を模倣しているか** の9点。
7〜9点目は 2026-09-01 追加——系譜 #11 が「パス基準を入力側と検査側で別々に決めた」形で、
#4 と #10 が「1周だけ回して完了にした」形、そして #11 が
**「テストの前提が実装のバグと一致していた」**形だったため。
...
2026-09-01 の修正後が初計測になる。件数を過去と比較しないこと。
...
**人間が Codex へ細かい修正を直接依頼したときに、誤って書き込んだもの**（本人の証言・2026-09-02）。
v2 のワークフローを回した結果ではない。

したがって **免責ブロックが破られた事例ではない**。
「実行中のエージェントがパスを取り違えた」という当初の疑いは**否定された**——
プロンプト経由で走る v2 のステップは、旧ディレクトリへ書いていなかった。

⚠️ ただし**リネームの動機は失われていない**。免責ブロックが必要だったのは
「同名ファイルを持つディレクトリが2つある」という配置そのものが原因で、
現にワークフロー外の手作業が間違えた。**配置を直せば注意書きが要らなくなる**、
という当初の判断は正しかった（注意書きが効いていたかどうかとは別の論点）。

---

## 計測の限界: codex の JSONL にタイムスタンプが無い（2026-09-02 判明）

**状態**: 未修正（外部 CLI の出力形式に依存するため、こちら側では直せない）。

### 内容

`runs/codex/*.jsonl` の各行のキーは `type` / `thread_id` / `item` だけで、
`item` 側も `id, type, text, command, aggregated_output, exit_code, status, changes, items`。
**時刻フィールドが 1 つも無い。**

そのため「イベントとイベントの間隔」を事後に集計できない。
無進行タイムアウトの閾値を決めるとき、これが直接の障害になった。

### 迂回した方法（同じ問題に当たったら再利用する）

無進行の正体は「1 コマンドの所要時間」である（`command_execution` は
`item.started` のあと、完了までイベントを出さない）。そこで**コマンドの実測所要**を
代理指標にした:

- `aggregated_output` に残る実行時間の表記（Playwright の `(Nm)`、`Time Elapsed` 等）を抽出
- エンジンが自分で測っている `verify-local` の所要（Event Log の message に埋まっている）
- タイムアウトした実行の「最後の JSONL 書込 → プロセス終了」（ファイル mtime と
  `exec.failed` の timestamp の差）。**これだけが「働いている最中の沈黙」の直接の実測**

3 つ目で得た **412.3s** が閾値の根拠になった。ただし打ち切られた実行の値なので**下界**。

### 影響と対処 ← **実測手段は入れた（2026-09-02）**

- **過去の実行**については間隔の分布は取れない。これは変わらない
- **以後の実行**は測れる。ウォッチドッグが `onProgress` の到達時刻から
  `maxIdleMs` / `progressEvents` / `topIdleMs`（上位5件）を算出し、
  `exec.completed` / `exec.failed` の meta へ記録する。**成功した実行でも記録する**
  ——「働いている実行はどこまで黙るのか」が知りたいのだから、分布の本体は成功側にある
- したがって**次の閾値見直しは実測分布から直接できる**。代理指標に戻らない
- (a) 働いていた / (b) 止まっていた の事後判定も、ファイル mtime 頼みでなくなる
  （`maxIdleMs` が閾値に張り付いていれば (b)、十分小さければ (a)）

⚠️ 記録先を sidecar ファイルではなく **Event Log の meta** にしたのは、新しい成果物
ファイルを足すと archive / 掃除のライフサイクルを抱えるため（KI-09 系譜 #10 で踏んだ形）。
Event Log は既に計測の一次資料になっており、archive にも随伴する。
全到達時刻の時系列が要るようになったら、そのときに sidecar を検討する。

⚠️ archive へコピーされた JSONL は `copyFileSync` により **mtime がコピー時刻になる**。
末尾ギャップの算出には `runs/codex/` の原本を使うこと。

---

## improve-check の判定範囲が Critical のみだった期間（〜2026-09-02）

**状態**: **解消済み**（判定範囲の拡大）。

### 内容

improve-check の Skill は「このステップが判定するのは **Critical が解消されたか**だけ」と
明記しており、`## Fix Scope` の `### Major` は判定対象外だった。

fix の後に review は**再走しない**（`fix → improve-check → reflection`）。
したがって **fix が Fix Scope の Major を直し損ねても、機構としては誰も止めない**。
Fix Scope は fix の契約なのに、契約の完了確認をする担当がいなかった。

### 実際にどう埋まっていたか

直近タスクでは、レビューセッションが Skill の要求を超えて**自主的に**差分を確認していた。
つまり穴は「善意で埋まっていた」だけで、**手順として保証されていなかった**。
KI-09 の系譜（宣言はあるが効いていない）とは逆向きの、
**「効いているが宣言が無い」**形である。担当者が代わればそのまま漏れる。

### 解消

判定対象へ `## Fix Scope` の `### Major` を追加し、三分岐にした:

| 状態 | 判定 |
| --- | --- |
| 解消（差分と実行結果で確認） | 通過 |
| **理由付き見送り**（`current-result.md` に対象と理由の記録あり） | 通過。reflection が backlog へ積む |
| **黙って未解消**（記述も理由も無い） | `fix-incomplete` |

⚠️ **許可値（2値）は増やしていない。** 第3の値にすると遷移表と validator に波及する
（エンジン変更になる）。通過側の値のまま、成果物へ見送り一覧を明記する形で表現した。

⚠️ **`fixAttempts` の上限（初回 + 2）も変えていない。** Major を含めると上限に
当たりやすくなるのは設計どおり——AI で収束しない Major は escalation で人間に渡すべき状況。
「理由付き見送り」の経路があることで、**正当な判断が escalation に吸い込まれること**は防ぐ。

### 連鎖して見つかった穴

reflection には**見送られた Major を backlog へ積む経路が無かった**。
`## Backlog` は fix より**前**に書かれる `current-review.md` にあり、
fix の見送り判断は `current-result.md` にしか無いためである。
経路を足さないと、見送りは improve-check を通過した時点で消える。

さらに backlog-rules の「`Trigger:` が無い項目は転記しない」と衝突した。
前者は「基準を満たさないので上げない」、後者は「**後でやると決めて通過させた**」で
性質が違うため、適用範囲を明示して分けた（backlog-rules v3）。

### 未計測

**fix が Major を取りこぼす頻度は測っていない。** 今回入れたのは「契約の完了確認」であって、
頻度に基づく対策ではない。fix 後の review 再走（往復 1 回の恒常コスト）は入れなかった——
漏れる実例が出てから再検討する。

## 兄弟機能のパリティ落ち（2026-09-04）

**状態**: **プロンプト層で対処**（research「兄弟機能の棚卸し」+ review「パリティ監査」）。
構造対処は BL-136（選択の正規化層）に起票、未着手。

### 内容

既存機能の類似物（兄弟）を後から足すとき、兄弟が持つ操作が新しい方へ引き継がれない。
仕様書は「兄弟と同じに動く」ことを書かないので、実装は書かれた範囲を正しく作って終わる。
逆方向（新しい方で足した操作が兄弟に入らない）も同型で起きる。
Fix 率の問題ではなく、**AC に載らないものは作られない**という当たり前の帰結。

### 実測（`JikkouYosanSakuseiGrid.tsx`、2026-09-04）

| 層 | 行選択 | セル選択 | 共通化 |
| --- | --- | --- | --- |
| 選択モデル | AG-Grid ネイティブ（`getSelectedRows()`） | 自前オーバーレイ（`cellRangeRef` + `extraCellsRef`） | 別物 |
| 選択の検出 | `useGridRowSelection` | `useGridDragRange` | hook ファイルは共通だが関数は別 |
| **操作の適用** | `getSelectedRows()` を消費する関数 ≈15 | `cellRangeRef` を消費する関数 5 | **完全に別** |
| モード分岐 | `shouldUseCellOperations()`。呼び出し元 1 箇所、**copy / cut / paste の 3 操作だけ** | | |

残り ≈12 の行操作はセル選択中「選択なし」として振る舞う。
さらに同じ 2 経路を `EditableGridCore.tsx`（共通コア）と `JikkouYosanSakuseiGrid.tsx`（画面固有）が
各々持つ。操作を両モードに効かせるには **2 モード × 2 グリッド = 4 箇所**。

### なぜプロンプト層だけで直らないか

棚卸しは「落とさない」ための網であって、4 箇所に足す手間そのものは減らない。
パリティが人力である限り落ちるのは構造的に必然なので、根本は BL-136（正規化層）。
Skill の棚卸しは、それまでの運用対処。

### 未計測

棚卸しの発動率（`## Sibling Parity` か「兄弟なし」の1行があるタスクの割合）と、
パリティ由来の Fix の件数。09-04 以前の review 指摘には「パリティ」の分類が無いので
遡及して数えられない。09-04 以降の review 指摘（research起因 Major）で数える。
