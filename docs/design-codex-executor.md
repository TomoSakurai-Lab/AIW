...
再検討する条件を先に書いておく。**どちらかが起きたら測り直す**:

- **Fix 率がソーク後 50% 超へ回帰**した
- **`fixAttempts` ≥2 のタスクが月 2 件を超えた**

⚠️ 判断の土台は n=20・CI ±19pt の観測値である。「95% から下がった」は CI を考慮しても
言えるが、「25% に定着した」はまだ言えない（M7 §7.1）。上の 2 条件は、
...
| 2026-09-04 | **C2（MCP 事前検証・課題E）** | **実装しない** | 前提だった Fix 率 95% がソークで **25%**（n=20・CI ±19pt）、`fixAttempts` は全タスク ≤1、fix 1周 6.0 分。呼び出し固定費 input 26,539 × 最大3回/step を払う理由が消えた。加えて `cacheRead / input` を動かしトークン基準線が前世代と接続しなくなる。再検討条件: **Fix 率が 50% 超へ回帰** または **`fixAttempts` ≥2 が月2件超**。仕様は課題E と `m3-design-inputs.md` §6 に残す（M7 §6.3） |
