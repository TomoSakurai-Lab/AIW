...
| 2026-09-02 | タイムアウト既定値の二重併存 | **統合は M4.4 の後**。今は据え置き | `CODEX_DEFAULT_TIMEOUT_MS`（30 分・codex.ts）と `DEFAULT_TOTAL_TIMEOUT_MS`（60 分・watchdog.ts）が併存している。codex.ts 不変の縛りを守った結果。**エンジン経由では必ず `req.timeoutMs` が埋まる**ことをテストで固定したので、前者は「死んでいると証明された既定」であり実害は無い。codex.ts に触れるようになったとき（M4.4 の抽象化判断の後）に片付ける |
