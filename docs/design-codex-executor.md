...
⚠️ **#8 の記述はもともと A-2 と食い違っていた**（「起動せずに停止」と要約していたが、
A-2 の本文は「外なら `--add-dir` を足す。判定できないときだけ起動しない」）。
実装は A-2 に従っており、上の表はテストを書く過程で訂正したもの。
**要約が本文と別のことを言う**のは、この文書で最初に見つかった不一致なので記録しておく。
