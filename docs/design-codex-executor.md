...
### 段階2（task-scoped resume）: **実装しない**（2026-08-21 確定）

着手判定は「段階1のトークン実測を見てから」と決めていた。実測が出たので**閉じる**。

**判定材料だった問い**: 「同一タスク内で implementation → fix のとき
コードベース再読み込みのトークンが浮くか」

**答え: 浮く余地がほとんど無い。** 根拠3点:

| # | 根拠 | 実測 |
| --- | --- | --- |
| 1 | **キャッシュが既に吸収している** | `cacheRead / input` = **93.9%**（n=20・92〜98%） |
| 2 | **課金に効くのは差引だけ** | 2,812,495 のうち実質 **109,135**（96% がキャッシュ） |
| 3 | **コストの支配項は往復回数**で、resume では減らない | items 15 → 66 に対し input 613K → 10.3M と比例。resume は「前回の文脈」を持ち越すだけで、**今回のタスクで何回ツールを呼ぶかは変えない** |

resume を入れれば、その代わりに
`taskRunId` の実装・不一致の拒否・破棄タイミング・fresh フォールバックという
**4つの機構と、それを守るテスト**を抱えることになる。得られるものが 6% 未満の削減では釣り合わない。

⚠️ **これは「後回し」ではなく「作らない」判断。** 再検討する条件を書いておく:
`cacheRead / input` の中央値が **80% を下回るようになったら**、前提が変わったので測り直す。

段階1（`--ephemeral` で fresh 固定）が M3 の最終形であり、
不変条件2（入力成果物だけから再実行できる）は**最適化のために緩めなかった**、という結果になった。

以下は判定材料として残す（実装しない条件の記録）。
...
## 課題E / C2: MCP 事前検証 ← **実装しない**（2026-09-01 確定）

着手判定は「executor の実測を見てから」と決めていた。M7 のソークで実測が出たので**閉じる**。

**C2 が解こうとしていた問題**: Fix 率 95% を、実装中の事前検証で下げること。

**答え: 問題の方が先に消えた。** M7 レポート §6.3 の対比:

| C2 が前提とした状況 | ソーク実測（n=20・08-21〜08-28） |
| --- | --- |
| Fix 率 95% — 事前検証で fix ループを減らしたい | **25%（5/20）**・95%CI ±19pt |
| fix ループが重い | fixAttempts は全タスク **≤1**、fix 1周の中央値 **6.0 分** |
| 事前に疑義を検出する仕組みが無い | sol の保守的停止が部分的に代替（3件・うち1件は実バグを発見） |

コスト側は測ってある: 呼び出しの固定費が **input 26,539**（trivial 実測）×
最大3回/step、加えてツール定義ぶんの固定費増。さらに `cacheRead / input` の比を動かすので、
**§4 のトークン基準線が前世代と接続しなくなる**（M2 世代の G-2 が交絡で判断不能になった形の再演）。

25% / ≤1 回 / 6.0 分に対して、毎ステップ最大3回の事前呼び出しと、それを支える
サーバー実装・回数制限・差分スキップ・Event Log 種別 4 つを抱えるのは釣り合わない。

⚠️ **これは「後回し」ではなく「作らない」判断**（段階2 resume と同じ扱い）。
再検討する条件を先に書いておく。**1つでも起きたら測り直す**:

| # | 条件 | 何を捉えるか |
| --- | --- | --- |
| 1 | **Fix 率が 60% 超へ回帰**した | 実装が一発で通らなくなった（C2 の当初の狙い） |
| 2 | **`fixAttempts` ≥2 のタスクが 20% を超えた** | fix 1周で収まらなくなった（ループの重さ） |
| 3 | **escalation が発生**した | fix で解けず人間へ上がった。**1件でも該当**——上2つは率で、これは事象で見る |

⚠️ **3 は上2つで代替できない。** 率は「回数」を見ているが、escalation は
「fix の枠内で解決できなかった」という質の違いで、少数でも起きた時点で
事前検証の価値が変わる。実測は **Event Log 全期間で 0 件**
（`workflow.halted` 44 件の内訳は `invalid-status` 27 / `validation-failed` 17。
`escalation` は 1 件も無い）なので、**0 からの立ち上がりをそのまま線にできる**。

⚠️ 判断の土台は n=20・CI ±19pt の観測値である。「95% から下がった」は CI を考慮しても
言えるが、「25% に定着した」はまだ言えない（M7 §7.1）。上の 3 条件は、
そこが崩れたときに自動で再検討へ戻すための線として置いている。

> ⚠️ **M7 §6.3 の「例」として挙げた 2 条件（50% 超 / 月2件超）は採らない。**
> あちらは推奨を書いた時点の叩き台で、確定判断では上の 3 条件を正本とする。
> 数値が食い違ったまま2箇所に残ると、次に見た人がどちらで判定するか決められない。

なお **`aiw_validate` の仕様は下に残す**。作らない判断であって、
仕様を捨てる判断ではない——再検討条件に触れたとき、設計からやり直さないため。
入力側の記録は `docs/m3-design-inputs.md` §6（**「実装する場合の仕様」として読むこと**）。

### 実装しないと決めた根拠（当初の「同時に入れない」理由・記録として残す）

以下は 2026-08-18 時点の「executor と同時に入れない」根拠。
上の確定判断はこれを引き継いだうえで、ソーク実測で**恒久的に閉じた**もの。
...
（当時の結論は「設計は M3 のうちに固め、実装だけを後ろに置く」だった。
実装を置いた先で、上の実測により**置くこと自体を取り消した**。）

### 実装する場合の仕様（作らないが、捨てない）
...
| 8 | `runtimeRoot` が `checkRepoRoot` の外 | **`--add-dir <runtimeRoot>` を足して救済**。`checkRepoRoot` を**解決できない**ときだけ起動せずに停止（A-2） |
...
### 全 9 件の実測状況（2026-08-21・完了）

| # | テスト | 備考 |
| --- | --- | --- |
| 1 | Test 104 | kill → 成果物だけで fresh 再実行 |
| 2 | Test 96 | タイムアウトは transient |
| 3 | Test 95 / 103 | exit 0 でも成果物は主張しない |
| 4 | Test 103 | file-exists が halt |
| 5 | Test 102 / 110 | Event Log・表示経路・`aiw log` に生 ID なし |
| 6 | Test 105 / 106 | clipboard へ戻せば従来動作 |
| 7 | **Test 114** | 401 は permanent。再試行に回さない |
| 8 | **Test 115** | 外なら `--add-dir` で救済、解決不能なら起動しない |
| 9 | Test 103 | exit 0 + 成果物なし |

### ⚠️ #8 は「設計より実装が周到だった」箇所 —— 上書きせずに経緯を残す

**設計時の想定（課題G の初版）**: `runtimeRoot` が `checkRepoRoot` の外なら **fail-fast**
（起動せずに停止）。

**実装（A-2 の本文に従った）**: **縮退継続** —— 外なら `--add-dir <runtimeRoot>` を足して
書けるようにし、`checkRepoRoot` を**解決できないときだけ**起動しない。

**後者を正とする理由**:

1. **fail-fast は救えるケースまで殺す。** runtimeRoot が外にあること自体は
   設定として妥当な構成（`settings.repoRoot` を明示した場合に起きる）で、
   `--add-dir` を足せば正しく動く。止める必要が無い
2. **縮退しても「見えなくなる」わけではない。** repoRoot の解決に失敗した場合は
   diff-scope が `skipped` を報告するので、**検査できていないことは画面と Event Log に出る**。
   静かな失敗ではない（M1.5 第1部の三値可視化がここで効いている）
3. 一方、**解決そのものができないときは縮退のしようがない**ので、そこは起動しない。
   「黙って書けない状態で走らせない」という A-2 の意図はここで満たされる

⚠️ **表だけ書き換えると「最初からこう設計した」と誤読される。**
diff-scope のときの「設計に未記述の判断4件を報告」と同じ扱いで、
**実装が設計を上回った箇所は上回ったと分かる形で残す**。
この文書で最初に見つかった、要約が本文と食い違っていた例でもある。

## 課題I: 実行の可視化（2026-08-18 追加）

clipboard 運用では、人間が Codex の対話画面で進行を見ていた。
executor 化は**この可視性を失う**副作用を持つ。放置すると
「30 分走っているが、何をしているのか分からない」状態になる。

### 段階1-1 に含める: 進行のストリーム表示（tee 方式）

`--json` の JSONL を `runs/` へ逐次保存するのと**同時に**、
イベント種別ごとの1行サマリを stderr へ流す。

```text
[12:03:15] thinking...
[12:03:42] edit: src/components/BudgetGrid.tsx
[12:04:01] shell: npx tsc --noEmit → exit 0
[12:05:30] tokens: in 26.5K / out 3.2K
```
...
| # | 要求 | 理由 |
| --- | --- | --- |
| 1 | **バッファして最後に出さない。逐次表示する** | 目的は進行の異常検知（今どのファイルを触っているか、テストを回しているか） |
| 2 | 思考・モデル出力の**全文は流さない**。種別と対象の1行要約のみ | 全文は `runs/` の JSONL にある。M5 で複数ステップ連続実行になると全文は読めない量になる |
| 3 | **生 session ID を含めない** | 防衛線2の grep テスト対象に**この表示経路も含める** |
...
aiw log <step> --raw    生 JSONL
aiw log <step> --json   機械可読（将来のモバイル UI 用のデータ口）
```

- データソースは**段階1-1 が保存する JSONL のみ**。新しい記録は作らない（読むだけ）
- `--json` の形式は `status --summary --json` と同じ方針（F2 のモバイル UI がそのまま食える形）
- ⚠️ **段階1-1〜1-3 の完了を優先する。故障注入9件の完了条件には含めない**

### 完了条件への追加

- 実タスク1本の実行中に、進行が**1行サマリでリアルタイムに見える**こと
- **生 session ID が表示経路に現れない**こと（grep テストの対象拡大）
...
| **1（後）** | drive も `step.executor` を解決する | ✅ 実装済み（2026-08-21） |

### 段階1 の設計（2026-08-21 に確定・それまで文書に無かった）

⚠️ この y/n の形は会話で決めていたが**文書に記録が無かった**。ここが正本。

```text
"implementation" は executor: codex を宣言しています。codex で実行しますか？
  y → codex executor を起動し、進行を1行サマリで流す（aiw exec と同じ）
  n → 従来どおりクリップボードへコピーして人手に渡す
```

| 決めたこと | 理由 |
| --- | --- |
| **確認を挟む**（自動起動しない） | drive は人間が1ステップずつ確認する経路。黙って走り出すのは drive の性格に反する |
| **n で clipboard へフォールバック** | 宣言を消さずにその場で従来運用へ逃げられる。不変条件5（ロールバック可能性）を運用面でも保つ |
| 進行表示は `aiw exec` と同じ | 表示の規則を2つ持たない（既定は codex の発言と error のみ） |
| clipboard 宣言のステップでは**何も聞かない** | 確認が毎ステップ出るとノイズになる |
...
| 2026-08-21 | 段階2（task-scoped resume） | **実装しない** | `cacheRead / input` 中央値 93.9%（n=20）で削減余地が 6% 未満。再検討条件: 中央値が 80% を下回ったら。課題D 参照 |
| 2026-09-02 | SIGTERM 後の居座り | **SIGKILL へのエスカレーションは作らない** | 実測で SIGTERM から実際の終了まで最大 237s（08-25 #1: 上限 1800s に対し実測 2037s）。ただし**居座りが実害になった記録はまだ無い**（次のタスクは開始できている）。作れば「まだ書いている最中のプロセスを強制終了する」経路を新設することになり、成果物が中途半端に残るリスクを引き受ける。**実害が出たら検討する**（再検討条件: 居座りが次タスクの開始を妨げた / ファイルロックが残った、のどちらかが観測されたとき） |
| 2026-09-02 | タイムアウト既定値の二重併存 | **統合は M4.4 の後**。今は据え置き（⚠️ **2026-09-17・M4.4 で判定**: 設定キーを `executorTimeoutMs` / `executorIdleTimeoutMs` へ中立化〔旧キーは 1 世代エイリアス + deprecation〕。`CODEX_DEFAULT_TIMEOUT_MS` と codex.ts の自前タイマーの整理は「executor 対称化の小枠」〔BL-221〕） | `CODEX_DEFAULT_TIMEOUT_MS`（30 分・codex.ts）と `DEFAULT_TOTAL_TIMEOUT_MS`（60 分・watchdog.ts）が併存している。codex.ts 不変の縛りを守った結果。**エンジン経由では必ず `req.timeoutMs` が埋まる**ことをテストで固定したので、前者は「死んでいると証明された既定」であり実害は無い。codex.ts に触れるようになったとき（M4.4 の抽象化判断の後）に片付ける |
| 2026-09-02 | 沈黙の実測 | **`maxIdleMs` / `progressEvents` / `topIdleMs` を exec の meta へ**。成功した実行でも記録する | 閾値の根拠が代理指標（コマンド所要）のままだと、次の見直しでも代理指標に戻る。エンジンは onProgress の到達時刻を既に見ているので、codex.ts に触らず測れる。⚠️ **失敗時だけ記録しない**——「働いている実行はどこまで黙るのか」が知りたいのだから、分布の本体は成功側にある。sidecar ファイルではなく Event Log の meta にしたのは、新しい成果物ファイルを足すと archive / 掃除のライフサイクルを抱えるため（KI-09 系譜 #10 で踏んだ形）。Event Log は既に計測の一次資料になっている |
| 2026-09-02 | タイムアウトの設計 | **二段構え**（総上限 3600s + 無進行 900s）。**ウォッチドッグは executor ではなくエンジン側の共通部品**（`engine/watchdog.ts`） | タイムアウト 5 件の JSONL 末尾判定が**全件 (a) 型 = 実作業中の打ち切り**で stall 0 件。総上限だけ延ばすと stall の発見が 30→60 分へ後退するので無進行を別に見張る。閾値 900s は実測（working silence 最大 412.3s ×2 + 余裕）から。300s は 08-25 #1 を誤 kill する。置き場所をエンジンにしたのは **M4 の claude.ts が同じ機構を無料で得る**ため——**M4.4「共通化すべきか」の比較表で、『実行時に参照される共通部品』の実例として1行になる**。codex.ts は不変（signal は配線済みだった） |
| 2026-09-01 | **C2（MCP 事前検証・課題E）** | **実装しない** | 前提だった Fix 率 95% がソークで **25%**（n=20・CI ±19pt）、`fixAttempts` は全タスク ≤1、fix 1周 6.0 分。呼び出し固定費 input 26,539 × 最大3回/step を払う理由が消えた。加えて `cacheRead / input` を動かしトークン基準線が前世代と接続しなくなる。再検討条件（1つでも該当したら測り直す）: **Fix 率 60% 超へ回帰** / **`fixAttempts` ≥2 が 20% 超** / **escalation 発生（1件でも）**。仕様は課題E と `m3-design-inputs.md` §6 に残す（M7 §6.3） |
| 2026-09-17 | **codex の env（BL-220）** | **許可リスト方式**（`CODEX_ENV_ALLOWLIST`・claude.ts と同じ 26 個・codex 固有の追加なし）。プロキシ / CA 証明書の変数は入れない（今の環境に無い。必要な環境では接続エラーとして失敗するので、そのとき実測して足す） | codex 0.147.0 の実体は `OPENAI_API_KEY` / `CODEX_API_KEY` / `CODEX_ACCESS_TOKEN` / `CODEX_AUTHAPI_BASE_URL` / `CODEX_URL` 等を読む（バイナリから抽出）。**実測（codex exec を実際に起動・親に偽の OPENAI_API_KEY / CODEX_API_KEY / ANTHROPIC_BASE_URL）**: 全継承（118 変数）→ `CODEX_API_KEY` が隔離 CODEX_HOME の ChatGPT ログインを**上書き**して 401・成果物なし。許可リスト（24 変数）→ 認証成功・成果物あり・3 変数とも子へ渡らない（27 秒）。PATH + SystemRoot のみ → 認証成功・成果物あり（44 秒）＝**起動できる最小**。最小まで削らないのは、implementation / fix が子として動かす npm / dotnet / git / Playwright の env 要件をファイル 1 つのプローブでは測れないため（このリストは review が dotnet build / nrun を実際に回している組み合わせ） |
| 2026-09-17 | **fix の codex への展開条件**（M5 の開始条件。⚠️ **投入は 2026-09-24**——09-17 はタスクの途中だったので境界まで待った。runtime の `versions.workflow` 7 → 8） | **実測 n≥2 で判定する**: Event Log に executor=codex・step=fix の `exec.completed` が 2 件以上たまった時点で下の観察手順で集計し、halt・失敗・異常が無ければ切り替えを確定する。あれば宣言の 1 行を消して clipboard へ戻す | ⚠️ runtime の implementation の宣言コメント（2026-08-31 から）は「fix の切り替えは別判定（展開条件は docs/design-codex-executor.md）」と書いていたが、**その展開条件はこの文書のどこにも書かれていなかった**（参照先が存在しない）。この行が最初の定義。fix は M3.3 の計画では implementation と同時に codex 化する対象だったが、宣言が無いまま 09-01 以降 35 回 clipboard で回っていた |

### fix × codex の観察手順（展開条件の集計・2026-09-17）

対象は **v8 以降**（runtime の `versions.workflow` 8 = fix に `executor: codex` を投入した世代）で fix が発生したタスク。
⚠️ 2026-09-17T09:56:05Z の fix の 3 イベントは `workflowVersion: 8` と記録されているが**実体は v7**（baseline の世代注記）なので除く。
情報源は `.ai-workflow/runs/execution-log.jsonl`（1 行 1 イベント）。タスクの境界は `transition` の `to: research`（task-planning の後）で切る。

| # | 観察 | 読むイベント・キー | 判定 |
| --- | --- | --- | --- |
| 1 | 実行の成否 | `exec.started` / `exec.completed` / `exec.failed` で `step == "fix"` かつ `executor == "codex"`。`failureKind`、`meta.timeoutKind`（total / idle / 無し）、`meta.exitCode`、`meta.errorEvents`、`meta.durationMs`、`meta.maxIdleMs` | `exec.failed` が 1 件でもあれば異常。⚠️ 外部中断（Ctrl+C）は `failureKind: transient` で `timeoutKind` 無し——rate limit 等と区別できない（M5 設計の課題） |
| 2 | diff-scope（fix は halt） | `validation.completed` の `results[]` で `type == "diff-scope"` の `status` / `message`。`validation.failed` の `validator == "diff-scope"`、`workflow.halted` | `failed` / halt は異常。⚠️ 宣言外ファイルの halt は BL-190（baseline がフェーズを跨いで更新されない）でも起きるので、`message` のファイル一覧が fix の変更か前フェーズの残りかを分けて読む |
| 3 | 安全網の status | 同じ `results[]` の `verify-local` / `measurement-completeness` / `json-schema`（`target == "ac-result.json"`・`"current-status.json"`）の `status`（passed / failed / skipped の三値。skipped を成功に潰さない） | failed は異常。skipped は理由（`message`）を報告 |
| 4 | fix 1 周の所要 | `exec.started`（fix・codex）→ 同じ fix の `transition`（`from: fix`）の時刻差。executor 分だけなら `exec.completed.meta.durationMs` | M3 の clipboard 中央値 **6.0 分**と並べる（人間の待ち時間を含む数え方の差を明記する） |
| 5 | BL-050 / BL-054 の再発 | fix の codex JSONL（`exec.completed.meta.jsonl` → `runs/codex/…jsonl`）の `command_execution` で、Volta の権限エラー（BL-050）/ `tools/nrun.cmd` がサンドボックスで動かない（BL-054）の症状。`current-result.md` の検証コマンドの記載 | 1 件でも再発なら報告 |
| 6 | 接続 | fix の `transition`（`fix -> improve-check`）→ improve-check の `transition`（`-> reflection`）→ reflection の `transition` が同じタスク内で揃うか。`fixAttempts` の値 | 途中で halt / escalation があれば異常 |
